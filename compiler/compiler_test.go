...
func runCompilerTests(t *testing.T, tests []compilerTestCase) {
	...
	for i, tt := range tests {
		...
		if err != nil {
			t.Fatalf("testInstructions failed on test %d: %s", i, err)
		}
		err = testConstants(t, tt.expectedConstants, bytecode.Constants)
		if err != nil {
			t.Fatalf("testConstants failed on test %d: %s", i, err)
		}
	}
}
...
func TestFunnctions(t *testing.T) {
	tests := []compilerTestCase{
		{
...
func TestCompilerScopes(t *testing.T) {
	...
	compiler.leaveScope()
	if compiler.scopeIndex != 0 {
		t.Errorf("scopeInex wrong. got=%d, want=%d", compiler.scopeIndex, 0)
	}
	if compiler.symbolTable != globalSymbolTable {
		t.Errorf("compiler did not restore global symbol table")
	}
	if compiler.symbolTable.Outer != nil {
		t.Errorf("compiler modified global symbole table incorrectly")
	}
